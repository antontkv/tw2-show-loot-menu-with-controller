...
class CContainer extends CGameplayEntity
{
	...
	//===ProjectMersey=== BEGIN - Glowing items without 'Quest' tag
	function IsGlowItem() : bool
	{
		var allItems		: array< SItemUniqueId >;
		var isGlow 			: bool;
		var i				: int;
		...
		for ( i=0; i<allItems.Size(); i+=1 )
		{
			if ( GetInventory().ItemHasTag(allItems[i], 'Glow') ) 
			{
				isGlow = true;
			}
		}
	...
	//===ProjectMersey=== END - Glowing items without 'Quest' tag
	...
	{	
		...
		if ( IsQuestItem() || IsGlowItem() )//===ProjectMersey=== - Glowing items without 'Quest' tag
		{
